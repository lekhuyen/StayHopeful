...
<div class="container mt-3">
    ...
    <form method="POST" enctype="multipart/form-data" action="{{ route('aboutuspage.update_teampic2', $teampic2Page) }}">
        @csrf
        @method("PUT")
        ...
        <div class="form-group mb-3">
            ...
            @if ($teampic2Page->images->count() > 0)
                @foreach ($teampic2Page->images as $item)
                    <img src="{{ asset($item->url_image) }}"
                         class="img-thumbnail"
                         width="100"
                         alt="{{ $teampic2Page->name }}"/>
                @endforeach
            @else
