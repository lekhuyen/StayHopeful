...
<div class="container mt-3">
    ...
    <table class="table table-dark mt-3" id="IntrocallTable">
        ...
        <tbody>
            @foreach ($leftcallPages as $introcallitem)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('aboutuspage.edit_leftcall', $introcallitem->id) }}" class="btn btn-warning">Edit</a>
                    </td>
                    <td>
                        <form action="{{ route('aboutuspage.delete_leftcall', $introcallitem->id) }}" method="POST" 
                            onsubmit="return confirm('Are you sure you want to delete this sector?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger">Delete</button>
        ...
        <div class="col" data-aos="fade-left" data-aos-duration="1000" data-aos-delay="400">
            @foreach ($leftcallPages as $leftcallitem)
                <div class="card h-100">
                    ...
                    <div class="card-body call_to_action_card">
                        <h5 class="card-title">{{ $leftcallitem->righttitle }}</h5>
                        <p class="card-text card-text-p">{{ $leftcallitem->rightdescription }}</p>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <table class="table table-dark mt-3" id="IntrocallTable">
        ...
        <tbody>
            @foreach ($teamPage as $teamitems)
                <tr>
                    <td>{{ $teamitems->id }}</td>
                    <td>{{ $teamitems->title }}</td>
                    <td>
                        <a href="{{ route('aboutuspage.detail', $teamitems->id) }}" class="btn btn-primary">View Description</a>
                    </td>
                    <td>
        ...
        <div class="row align-items-center aboutus_johndoe">
            ...
            <div class="col-md-4 order-md-3 right_picture">
                @foreach ($teampic2Page as $pic2items) 
                    ...
                    @endif
                    <div class="d-grid gap-2 d-md-block">
                        <a href="{{ route('aboutuspage.edit_teampic1', $pic2items->id) }}" class="btn btn-warning">Edit</a>
                        <form action="{{ route('aboutuspage.delete_teampic1', $pic2items->id) }}" method="POST" 
                            onsubmit="return confirm('Are you sure you want to delete this picture?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger">Delete</button>
...
<div class="container mt-3">
    <h4>Main Question Sector</h4>
    <a href="{{ route('aboutuspage.create_mainquestion') }}" class="btn btn-primary">Add Main Question Sector</a>
    <table class="table table-dark mt-3" id="mainTable">
        <thead>
            <tr>
                <th>Id</th>
        ...
        <tbody>
            @foreach ($mainquestionPage as $mainquestionitem)
            <tr>
                <td>{{ $mainquestionitem->id }}</td>
                <td>{{ $mainquestionitem->title }}</td>
                <td>
                  <a href="{{ route('aboutuspage.detail', $mainquestionitem->id) }}" class="btn btn-primary">View Description</a>
                </td>
                <td>
                    <a href="{{ route('aboutuspage.edit_mainquestion', $mainquestionitem->id) }}" class="btn btn-warning">Edit</a>
                </td>
                <td>
                    <form action="{{ route('aboutuspage.delete_mainquestion', $mainquestionitem->id) }}" method="POST"
