@foreach ($comment as $reply)
    <div id="comment_reply-post comment_post" class="reply_comment-post" data-id="{{ $reply->id }}"
        style="margin-left: -19px; margin-top: 10px; display: flex">
        <a href="">
            ...
            @foreach ($user as $item)
                @if (!$check)
                    @if ($reply->user_id == $item->id && $item->avatar != null)
                        <img src="{{ asset($item->avatar) }}" alt="" width="50"
                            style=" width: 80px;clip-path: circle(30%);">
                        @php
                            $check = true;
                        @endphp
                    @elseif ($reply->user_id == $item->id && $item->avatar == null)
        ...
        <div class="comment_body">
            <div class="comment_background" data-id="{{ $reply->id }}">
                ...
                <div class="delete_comment-post delete_comment-reply-post">
                    ...
                    <div class="edit_delete-post" style="display: none" data-id="{{ $reply->id }}">
                        <p class="report__reply" data-id="{{ $reply->id }}">Report</p>
                    </div>
                </div>
                @endif
            ...
            <form style="display: none; margin-bottom: 10px;"
                class="form_reply-{{ $reply->id }} form_reply btn_reply-submit btn_edit-reply show_form-edit-reply"
                data-id="{{ $reply->id }}">
                <div id="input_reply-comment" style="width: 90%;">
                    <textarea style="padding: 10px 50px 10px 20px;" cols="" rows="10" name="content" placeholder="comment.."
                        class="content_reply-{{ $reply->id }} content_reply" data-id="{{ $reply->id }}"></textarea>
